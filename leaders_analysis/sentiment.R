...
# df
df = read_excel("leaders_bert.xlsx")

my_text = df['bert_text']


# Tokenize sentences for each row
my_text$sentences <- lapply(my_text$bert_text, function(x) get_tokens(x)) # tokenize_sentences
...
# save df
write_xlsx(df, 'leaders_sentiment_syuzhet.xlsx')



# plot
plot = df %>% arrange(kuupäev) 
